...
public class Chat {
	...
	private static final char TRANSPARENT_CHAR = ' ';
	  ...
	  public Chat(BufferedImage image, int height, char imgChar)
	  {
	    ChatColor[][] chatColors = toChatColorArray(image, height);
	    this.lines = toImgMessage(chatColors, imgChar);
	  }

	  public Chat(ChatColor[][] chatColors, char imgChar) {
	    this.lines = toImgMessage(chatColors, imgChar);
	  }

	  public Chat(String[] imgLines) {
	    this.lines = imgLines;
	  }

	  public Chat appendText(String[] text) {
	    for (int y = 0; y < this.lines.length; y++) {
	      if (text.length > y)
	      {
	        int tmp16_15 = y;
	        String[] tmp16_12 = this.lines; tmp16_12[tmp16_15] = (tmp16_12[tmp16_15] + " " + text[y]);
	      }
	    }
	    return this;
	  }

	  public Chat appendCenteredText(String[] text) {
	    for (int y = 0; y < this.lines.length; y++) {
	      if (text.length > y) {
